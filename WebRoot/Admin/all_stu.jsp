    ...
    <div id="body">
		...
		<div id="content">
			<div id="content-left">
				...
				<ul id="ul">
					<li class="selected"><a href="all_stu.jsp">所有课程</a></li>
				...
				<div id="content-content">
					<div class="list">
						<table id="table">
							...
							<jsp:setProperty property="studentid" name="selcourse" value="<%=loginuser.getUserid()%>" />
							<c:forEach items="${classbean.allclass}" var="varclass" begin="${classbean.page*10-10}" end="${classbean.page*10-1}" varStatus="status" >
								<jsp:setProperty property="teacher" name="Class" value="${varclass.teacher}" />
								<jsp:setProperty property="classid" name="selcourse" value="${varclass.id}" />
								<c:if test="${status.index < classbean.size}">
									<tr align='center' class="tr">
										...
										<td>
											<c:set var="flag" value="<%=new SelCourseDAO().isExist(selcourse.getClassid(),selcourse.getStudentid())%>"></c:set>
											<c:if test="${flag==true}">
												<a>不可选</a>
											</c:if>
											<c:if test="${flag==false}">
												<a href='../StuAlter?alter=1&classid=${varclass.id}'>选课</a>
											</c:if>
					...
					<div class="pagegroup" align="center">
							   ...
							   <c:if test="${flag==false}">
							   		<a href='all_stu.jsp'>首页</a>
							   		<a href='all_stu.jsp?page=${classbean.page-1}'>上一页</a>
							   </c:if>
							   <c:if test="${classbean.page==classbean.allpage}" var="flag1"></c:if>
							    ...
							    <c:if test="${flag1==false}">
							   		<a href='all_stu.jsp?page=${classbean.page+1}'>下一页</a>
							   		<a href='all_stu.jsp?page=${classbean.allpage}'>尾页</a>
							   </c:if>
					 </div>
				</div>
