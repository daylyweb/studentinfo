...
<jsp:useBean id="selcoursebean" scope="page" class="com.daylyweb.studentinfo.beans.SelCourseBean">
	...
	<jsp:setProperty property="classtype" name="selcoursebean" value="2" />
		...
		<div id="content">
			<div id="content-left">
				...
				<ul id="ul">
					...
					<li class="selected"><a href="ok_stu.jsp">已修课程</a></li>
				...
				<div id="content-content">
							...
							<c:forEach items="${selcoursebean.classes}" var="varclass" begin="${selcoursebean.page*10-10}" end="${selcoursebean.page*10-1}" varStatus="status" >
								...
								<c:if test="${status.index < selcoursebean.size}">
									<tr align='center' class="tr">
										...
										<td>
											<%=new SelCourseDAO().queryGrade(selcoursebean.getClassid(), selcoursebean.getStudentid()) %>
										</td>
									</tr>
								</c:if>
							</c:forEach>
					...
					<div class="pagegroup" align="center">
							   ...
							   <c:if test="${flag==false}">
							   		<a href='ok_stu.jsp'>首页</a>
							   		<a href='ok_stu.jsp?page=${selcoursebean.page-1}'>上一页</a>
							   </c:if>
							   <c:if test="${selcoursebean.page==selcoursebean.allpage}" var="flag1"></c:if>
							    ...
							    <c:if test="${flag1==false}">
							   		<a href='ok_stu.jsp?page=${selcoursebean.page+1}'>下一页</a>
							   		<a href='ok_stu.jsp?page=${selcoursebean.allpage}'>尾页</a>
							   </c:if>
					 </div>
				</div>
